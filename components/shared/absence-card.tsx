...
export function AbsenceCard() {
  const { openAbsenceView, getUnseenProcessedRequestsCount, markMyRequestsAsSeen } = useAbsenceStore();
  const { currentUser } = useAuthStore();

  const unseenCount = currentUser ? getUnseenProcessedRequestsCount(currentUser.id) : 0;

  const handleClick = () => {
    if (currentUser) {
      markMyRequestsAsSeen(currentUser.id);
    }
    openAbsenceView();
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-rose-50 w-20 h-20 sm:w-28 sm:h-28 lg:w-[var(--module-icon-size)] lg:h-[var(--module-icon-size)] rounded-2xl sm:rounded-3xl lg:rounded-[var(--module-icon-radius)] flex items-center justify-center transition-all duration-500 group-hover:scale-105 group-hover:rotate-1 z-10" aria-hidden="true">
        ...
        <div className="relative text-rose-500 transition-transform duration-500 group-hover:scale-105">
          <Umbrella className="w-10 h-10 sm:w-14 sm:h-14 lg:w-[var(--module-icon-inner)] lg:h-[var(--module-icon-inner)]" strokeWidth={1.2} />
        </div>
      </div>
      <div className="flex flex-col items-center space-y-3 z-10">
        ...
        <div className="w-8 h-0.5 bg-slate-200 rounded-full group-hover:w-16 group-hover:bg-rose-500 transition-all duration-500 opacity-50" aria-hidden="true" />
      </div>
    </button>
  );
}
