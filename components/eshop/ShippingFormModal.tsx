...
export function ShippingFormModal() {
  const { editingShippingId, selectedShopId, closeShippingForm, createShipping, updateShipping, shopShippings } = useEshopEshopyStore();
  const existing = editingShippingId ? shopShippings.find((s) => s.id === editingShippingId) : null;

  const [saving, setSaving] = useState(false);
  const [name, setName] = useState(existing?.name ?? '');
  const [type, setType] = useState(existing?.type ?? 'zasilkovna');
  const [price, setPrice] = useState(existing?.price ?? 0);
  const [freeFrom, setFreeFrom] = useState<number | ''>(existing?.freeFrom ?? '');
  ...
  const handleSubmit = async () => {
    ...
    const data = {
      ...
      type: type.trim(),
      price,
      freeFrom: freeFrom === '' ? undefined : freeFrom,
      active,
      order,
      config: {},
    };
    ...
    if (editingShippingId) {
      await updateShipping(editingShippingId, data);
    } else {
      await createShipping({ ...data, shopId: selectedShopId });
    }
    setSaving(false);
  };
      ...
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-md animate-in slide-in-from-top-2 duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h2 className="text-lg font-bold text-slate-800">
            {existing ? 'Upravit dopravu' : 'Nová doprava'}
          </h2>
          <button onClick={closeShippingForm} className="p-2 rounded-lg hover:bg-slate-100 transition-colors">
        ...
        <div className="px-6 py-4 space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputClass} placeholder="Zásilkovna" />
          </div>
          <div>
            ...
            <select value={type} onChange={(e) => setType(e.target.value)} className={inputClass}>
              ...
              <option value="osobni_odber">Osobní odběr</option>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={labelClass}>Zdarma od (Kč)</label>
              <input
                type="number"
                value={freeFrom}
                onChange={(e) => setFreeFrom(e.target.value === '' ? '' : Number(e.target.value))}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200">
          <button onClick={closeShippingForm} className="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 transition-colors">
            Zrušit
          </button>
          <button
