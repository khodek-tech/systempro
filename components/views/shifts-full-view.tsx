...
import { ArrowLeft, ChevronLeft, ChevronRight, Building2, User } from 'lucide-react';
...
export function ShiftsFullView() {
  ...
  // Determine if user uses their own working hours or store hours
  const usesOwnWorkingHours = currentUser && currentUser.storeIds.length === 0 && currentUser.workingHours;
  const usesStoreHours = currentUser && currentUser.storeIds.length > 0;
  ...
  return (
    <main className="flex-1 p-6 overflow-y-auto bg-slate-50">
      <div className="space-y-6 pb-16 animate-in fade-in duration-300">
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    {cell.isWorkDay && (
                      <div className="mt-auto">
                        ...
                        >
                          {cell.isShortWeek ? 'Krátký' : 'Dlouhý'}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        {(usesOwnWorkingHours || usesStoreHours) && (
          <div className="flex items-center justify-center gap-2 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
            {usesOwnWorkingHours ? (
              ...
              <>
                ...
                <span className="text-sm font-medium text-green-700">Pracovní doba: Dle prodejny</span>
              </>
            )}
          </div>
        )}
        ...
        <div className="flex items-center justify-center space-x-6 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-slate-600">St, Čt</span>
          </div>
          <div className="flex items-center space-x-2">
            <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase bg-blue-100 text-blue-700">
              Dlouhý
            </span>
            <span className="text-sm text-slate-600">Po, Út, Pá, So, Ne</span>
          </div>
        </div>
      </div>
    </main>
