...
export function CollectModule() {
  const { collectModalOpen, setCollectModalOpen } = useUIStore();

  const { cashToCollect, getCollectionPeriod, submitCollection } = useSalesStore();
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative bg-blue-50 w-20 h-20 sm:w-28 sm:h-28 lg:w-[var(--module-icon-size)] lg:h-[var(--module-icon-size)] rounded-2xl sm:rounded-3xl lg:rounded-[var(--module-icon-radius)] flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 z-10">
          ...
          <div className="relative text-blue-500 transition-transform duration-500 group-hover:scale-110">
            <Truck className="w-10 h-10 sm:w-14 sm:h-14 lg:w-[var(--module-icon-inner)] lg:h-[var(--module-icon-inner)]" strokeWidth={1.2} />
          </div>
        </div>
