...
export function PresenceModule() {
  ...
  return (
    <button
      onClick={openPresenceView}
      aria-label={`Přítomnost - ${presentCount}/${records.length}`}
    ...
    >
      ...
      />
      <div
        className="relative bg-blue-50 w-20 h-20 sm:w-28 sm:h-28 lg:w-[var(--module-icon-size)] lg:h-[var(--module-icon-size)] rounded-2xl sm:rounded-3xl lg:rounded-[var(--module-icon-radius)] flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 z-10"
        aria-hidden="true"
      >
        ...
        <div className="relative text-blue-500 transition-transform duration-500 group-hover:scale-110">
          <Users className="w-10 h-10 sm:w-14 sm:h-14 lg:w-[var(--module-icon-inner)] lg:h-[var(--module-icon-inner)]" strokeWidth={1.2} />
        </div>
      </div>
      <div className="flex flex-col items-center space-y-3 z-10">
        <div className="flex items-center gap-3">
          ...
          {records.length > 0 && (
            <span
              className="inline-flex items-center justify-center min-w-[28px] h-7 px-2 rounded-full text-sm font-bold bg-blue-500 text-white"
              aria-label={`${presentCount} z ${records.length} přítomných`}
            >
        ...
        <div
          className="w-12 h-1.5 bg-slate-100 rounded-full group-hover:w-20 group-hover:bg-blue-500 transition-all duration-500 opacity-50"
          aria-hidden="true"
        />
      </div>
    </button>
