...
export function ChatModule() {
  const { openChatView, getUnreadCountForUser } = useChatStore();
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="relative bg-rose-50 w-20 h-20 sm:w-28 sm:h-28 lg:w-[var(--module-icon-size)] lg:h-[var(--module-icon-size)] rounded-2xl sm:rounded-3xl lg:rounded-[var(--module-icon-radius)] flex items-center justify-center transition-all duration-500 group-hover:scale-105 group-hover:rotate-1 z-10"
        aria-hidden="true"
      >
      ...
      <div className="flex flex-col items-center space-y-3 z-10">
        <div className="flex items-center gap-3">
          ...
          {unreadCount > 0 && (
            <span
              ...
              aria-label={`${unreadCount} nepřečtených zpráv`}
            >
              {unreadCount}
            </span>
          )}
        ...
        <div
          className="w-8 h-0.5 bg-slate-200 rounded-full group-hover:w-16 group-hover:bg-rose-500 transition-all duration-500 opacity-50"
          aria-hidden="true"
        />
      </div>
    </button>
